...
public class StoryRepository {
    ...
    public void upsert(JiraIssue issue) {
        String updateSql = """
                ...
                WHERE story_key = ?
                """;
        int updated = jdbcTemplate.update(updateSql, issue.summary(), issue.status(), issue.epicKey(), issue.workType(), issue.issueKey());
        if (updated == 0) {
            String insertSql = """
                    INSERT INTO stories (id, story_key, summary, status, epic_key, work_type)
                    VALUES (?, ?, ?, ?, ?, ?)
                    """;
            jdbcTemplate.update(insertSql, java.util.UUID.randomUUID(), issue.issueKey(), issue.summary(), issue.status(), issue.epicKey(), issue.workType());
        }
    }
